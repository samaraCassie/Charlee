# V3.1 - Integration Layer Implementation

## 📅 Data de Implementação
2025-01-15

## 🎯 Objetivo
Criar uma camada de integração completa que permite comunicação assíncrona entre todos os módulos do Charlee, mantendo visão holística do estado do usuário e habilitando decisões context-aware.

## 🏗️ Arquitetura Implementada

### 1. Event Bus (Sistema Pub/Sub)

**Arquivo:** `backend/integration/event_bus.py`

Sistema de eventos que permite comunicação assíncrona entre módulos sem acoplamento forte.

**Componentes:**
- `Event` - Dataclass para eventos
- `EventBus` - Classe principal com pub/sub
- `EventType` - Enum com todos os tipos de eventos
- `ModuleName` - Enum com nomes de módulos

**Features:**
- ✅ Publicação de eventos no banco de dados
- ✅ Integração opcional com Redis para real-time
- ✅ Fila de processamento assíncrona
- ✅ Sistema de prioridades
- ✅ Subscribers com callbacks
- ✅ Estatísticas de eventos

**Tabela:** `system_events`
- Armazena todos os eventos do sistema
- Índices otimizados para tipo, prioridade e timestamp
- Tracking de processamento

### 2. Context Manager

**Arquivo:** `backend/integration/context_manager.py`

Mantém visão holística do estado atual do usuário para decisões context-aware.

**Componentes:**
- `ContextManager` - Classe principal
- `GlobalContext` - Model de banco de dados

**Features:**
- ✅ Snapshot de estado atual (wellness, capacity, focus)
- ✅ Subscrição automática a eventos relevantes
- ✅ Atualização automática de contexto
- ✅ Decisões context-aware:
  - `should_accept_interruption()`
  - `get_optimal_activity_type()`
  - `needs_break()`

**Tabela:** `global_context`
- Um registro por usuário (snapshot atual)
- Inclui: fase do ciclo, energia, carga de trabalho, stress, etc.
- Atualizado em tempo real via eventos

### 3. Cross-Module Relations

**Tabela:** `cross_module_relations`

Sistema para rastrear relacionamentos entre entidades de diferentes módulos.

**Exemplos:**
- `project_to_task` - Projeto freelance → Tarefas criadas
- `notification_to_task` - Notificação → Tarefa criada
- `task_to_okr` - Tarefa → OKR relacionado

### 4. Decisões Integradas

**Tabela:** `integrated_decisions`

Registro de decisões que envolvem múltiplos módulos.

**Features:**
- Situação que requer decisão
- Módulos envolvidos
- Contexto considerado
- Opções avaliadas
- Decisão tomada + justificativa
- Tracking de execução

### 5. Integrações Específicas

#### Task ↔ Wellness Integration

**Arquivo:** `backend/integration/task_wellness_integration.py`

- Ajusta prioridades de tarefas baseado em fase do ciclo
- Recomenda tarefas leves durante baixa energia
- Filtra tarefas por wellness state
- Método: `get_wellness_adjusted_tasks()`

#### Capacity Integration

**Arquivo:** `backend/integration/capacity_integration.py`

- Proteção contra sobrecarga
- Sugestões de adiamento quando capacity > 90%
- Análise antes de aceitar novos trabalhos
- Método: `check_capacity_before_accept()`

### 6. User Settings Persistence

**Arquivo:** `backend/api/routes/settings.py`
**Tabela:** `user_settings`

✅ **TODO Resolvido:** Settings agora são salvos no banco de dados

**Features:**
- Persistência de preferências do usuário
- Display preferences (theme, density)
- Notification preferences
- Work preferences (horas/semana)
- Wellness preferences (tracking de ciclo)
- Integration settings (JSON flexível)

## 📊 Schemas de Banco de Dados

### Migration: `004_add_integration_layer.py`

**Tabelas Criadas:**
1. `user_settings` - Configurações do usuário
2. `system_events` - Eventos do sistema
3. `global_context` - Contexto holístico
4. `cross_module_relations` - Relacionamentos cross-module
5. `integrated_decisions` - Decisões integradas

**Índices Criados:**
- Índices simples em todos os IDs e foreign keys
- Índices compostos para queries comuns:
  - `(tipo, processado)` em system_events
  - `(prioridade DESC, criado_em)` em system_events
  - `(modulo_origem, entidade_origem_id)` em cross_module_relations
  - `(modulo_destino, entidade_destino_id)` em cross_module_relations

## 🎯 Event Types Disponíveis

### Task Manager
- `TASK_CREATED`, `TASK_UPDATED`, `TASK_COMPLETED`, `TASK_DELETED`
- `TASK_DEADLINE_APPROACHING`, `TASK_OVERDUE`

### Big Rocks
- `BIG_ROCK_CREATED`, `BIG_ROCK_UPDATED`, `BIG_ROCK_CAPACITY_CHANGED`

### Wellness Coach
- `CYCLE_PHASE_CHANGED`, `CYCLE_LOGGED`
- `ENERGY_LOW`, `ENERGY_HIGH`, `WELLNESS_ALERT`

### Capacity Guardian
- `CAPACITY_WARNING`, `CAPACITY_CRITICAL`
- `OVERLOAD_DETECTED`, `CAPACITY_NORMALIZED`

### Focus Module (Future)
- `FOCUS_SESSION_STARTED`, `FOCUS_SESSION_ENDED`
- `NOTIFICATION_URGENT`, `INTERRUPTION_BLOCKED`

### System
- `CONTEXT_UPDATED`, `DECISION_REQUIRED`, `DECISION_EXECUTED`

## 🧪 Testes

**Arquivo:** `backend/tests/integration/test_event_bus.py`

**Testes Criados:**
- ✅ `test_publish_event` - Publicação de eventos
- ✅ `test_event_subscription` - Sistema de subscrição
- ✅ `test_event_processing` - Processamento assíncrono
- ✅ `test_get_recent_events` - Busca de eventos
- ✅ `test_event_stats` - Estatísticas de eventos

## 📈 Métricas e Stats

### Event Bus Stats
```python
stats = event_bus.get_event_stats(hours=24)
# Returns:
# {
#     "total_events": 150,
#     "processed_events": 147,
#     "unprocessed_events": 3,
#     "processing_rate": 98.0,
#     "events_by_type": {...},
#     "events_by_module": {...}
# }
```

## 🔄 Fluxo de Eventos

```
1. Módulo publica evento via EventBus.publish()
   ↓
2. Evento é salvo no DB (system_events)
   ↓
3. Evento é publicado no Redis (se disponível)
   ↓
4. Evento é adicionado à fila de processamento
   ↓
5. EventBus processa e notifica subscribers
   ↓
6. Subscribers executam suas ações
   ↓
7. Evento marcado como processado
```

## 📚 Exemplos de Uso

### Publicar um Evento

```python
from integration.event_bus import Event, EventBus
from integration.event_types import EventType, ModuleName

event_bus = EventBus(db_session, redis_client)

await event_bus.publish(Event(
    tipo=EventType.TASK_COMPLETED,
    modulo_origem=ModuleName.TASK_MANAGER,
    payload={"task_id": 123, "user_id": 1},
    prioridade=5
))
```

### Subscribir a Eventos

```python
def on_task_completed(event: Event):
    task_id = event.payload["task_id"]
    print(f"Task {task_id} completed!")

event_bus.subscribe(EventType.TASK_COMPLETED, on_task_completed)
event_bus.start_processing()
```

### Usar Context Manager

```python
from integration.context_manager import ContextManager

context_manager = ContextManager(db_session, event_bus, user_id=1)

# Get current context
context = context_manager.get_context()
print(f"Energia atual: {context['energia_atual']}/10")
print(f"Fase do ciclo: {context['fase_ciclo']}")

# Check if can accept interruption
if context_manager.should_accept_interruption():
    print("OK para interromper")
else:
    print("Não interrompa - usuária em foco ou baixa energia")

# Get optimal activity
activity = context_manager.get_optimal_activity_type()
print(f"Atividade ideal: {activity}")
```

### Integração Task-Wellness

```python
from integration.task_wellness_integration import TaskWellnessIntegration

integration = TaskWellnessIntegration(db_session, event_bus, context_manager)

# Get wellness-adjusted task recommendations
recommendations = integration.get_wellness_adjusted_tasks(limit=10)

print(f"Tarefas recomendadas: {len(recommendations['recommended'])}")
print(f"Tarefas adiadas: {len(recommendations['deferred'])}")
print(f"Razão: {recommendations['reason']}")
```

## 🚀 Próximos Passos

Com a Integration Layer implementada, agora é possível:

1. **Módulo de Notificações** - Usar Event Bus para filtrar notificações
2. **Projetos Freelance** - Publicar eventos de projetos e integrar com tasks
3. **OKR Dashboard** - Rastrear progresso via eventos de tasks
4. **Analytics Avançado** - Consumir eventos para métricas em tempo real
5. **Automações** - Criar workflows baseados em eventos

## 📊 Impacto

**Antes (V3.0):**
- Módulos isolados, sem comunicação
- Decisões locais, sem contexto global
- Dados em silos

**Depois (V3.1):**
- ✅ Comunicação assíncrona via eventos
- ✅ Decisões context-aware
- ✅ Visão holística do estado do usuário
- ✅ Base para features avançadas
- ✅ Escalabilidade para novos módulos

## 🎓 Lições Aprendidas

1. **Event Bus é poderoso** - Desacopla módulos e permite evolução independente
2. **Context é essencial** - Decisões melhores quando consideram estado global
3. **Async é necessário** - Processamento assíncrono evita bloqueios
4. **Redis opcional** - Sistema funciona sem, mas Redis adiciona real-time
5. **Testes são críticos** - Integração complexa requer testes robustos

---

**Versão:** 3.1.0
**Status:** ✅ Implementado
**Data:** 2025-01-15
