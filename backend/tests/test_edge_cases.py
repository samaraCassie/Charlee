...
class TestBigRockEdgeCases:
    ...
    def test_create_big_rock_with_unicode_name(self, client):
    ...
    def test_create_big_rock_with_max_length_name(self, client):
    ...
    def test_create_big_rock_with_special_characters(self, client):
    ...
    def test_create_big_rock_with_null_color(self, client):
    ...
    def test_update_big_rock_with_empty_payload(self, client, sample_big_rock):
...
class TestTaskEdgeCases:
    ...
    def test_create_task_with_very_long_description(self, client):
    ...
    def test_create_task_with_newlines(self, client):
    ...
    def test_create_task_with_past_deadline(self, client):
    ...
    def test_create_task_with_far_future_deadline(self, client):
    ...
    def test_list_tasks_with_large_limit(self, client, sample_task):
        """Should handle large limit values gracefully."""
        response = client.get("/api/v1/tasks?limit=10000")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "tasks" in data

    def test_list_tasks_with_large_skip(self, client):
        """Should handle large skip values without error."""
        response = client.get("/api/v1/tasks?skip=100000")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_complete_already_completed_task(self, client, sample_task):
        ...
        # First completion
        client.post(f"/api/v1/tasks/{sample_task.id}/complete")

        # Second completion
        response = client.post(f"/api/v1/tasks/{sample_task.id}/complete")
        assert response.status_code == status.HTTP_200_OK

    def test_reopen_pending_task(self, client, sample_task):
        """Should handle reopening a task that isn't completed."""
        response = client.post(f"/api/v1/tasks/{sample_task.id}/reopen")
...
class TestConcurrencyEdgeCases:
    ...
    def test_create_multiple_big_rocks_same_name(self, client):
        """Should allow multiple Big Rocks with same name."""
        big_rock_data = {"name": "Duplicate Name", "color": "#111111"}

        response1 = client.post("/api/v1/big-rocks", json=big_rock_data)
        response2 = client.post("/api/v1/big-rocks", json=big_rock_data)

        assert response1.status_code == status.HTTP_201_CREATED
        assert response2.status_code == status.HTTP_201_CREATED
        assert response1.json()["id"] != response2.json()["id"]

    def test_delete_same_task_twice(self, client, sample_task):
        """Should handle double deletion gracefully."""
        # First deletion
        response1 = client.delete(f"/api/v1/tasks/{sample_task.id}")
        assert response1.status_code == status.HTTP_204_NO_CONTENT

        # Second deletion should fail
        response2 = client.delete(f"/api/v1/tasks/{sample_task.id}")
...
class TestInputBoundaryConditions:
    ...
    def test_create_task_with_empty_string_description(self, client):
    ...
    def test_create_task_with_whitespace_only_description(self, client):
    ...
    def test_create_big_rock_with_invalid_color_format(self, client):
    ...
    def test_create_big_rock_with_hex_without_hash(self, client):
