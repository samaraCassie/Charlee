...
def upgrade() -> None:
    ...
    op.create_index("idx_tasks_user_deadline", "tasks", ["user_id", "deadline"], unique=False)
    ...
    op.create_index("idx_tasks_big_rock_status", "tasks", ["big_rock_id", "status"], unique=False)

    # BigRocks table
    # Optimize: SELECT * FROM big_rocks WHERE user_id = ? AND active = true
    op.create_index("idx_big_rocks_user_active", "big_rocks", ["user_id", "active"], unique=False)
    ...
    op.create_index("idx_daily_logs_user_date", "daily_logs", ["user_id", "log_date"], unique=False)
    ...
    op.create_index(
        "idx_calendar_events_user_start", "calendar_events", ["user_id", "start_time"], unique=False
    )
    ...
    op.create_index(
        "idx_notifications_user_read", "notifications", ["user_id", "read_at"], unique=False
    )
    ...
    op.create_index(
        "idx_attachments_task_type", "attachments", ["task_id", "file_type"], unique=False
    )

    # Optimize: SELECT * FROM attachments WHERE user_id = ? ORDER BY created_at DESC
    op.create_index(
        "idx_attachments_user_created", "attachments", ["user_id", "created_at"], unique=False
    )
