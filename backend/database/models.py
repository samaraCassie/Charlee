...
class Tarefa(Base):
    ...
    def __repr__(self):
        return (
            f"<Tarefa(id={self.id}, descricao='{self.descricao[:30]}...', status='{self.status}')>"
        )
...
class CicloMenstrual(Base):
    ...
    nivel_energia = Column(
        Integer, CheckConstraint("nivel_energia BETWEEN 1 AND 10"), nullable=True
    )
    nivel_foco = Column(Integer, CheckConstraint("nivel_foco BETWEEN 1 AND 10"), nullable=True)
    nivel_criatividade = Column(
        Integer, CheckConstraint("nivel_criatividade BETWEEN 1 AND 10"), nullable=True
    )
...
class RegistroDiario(Base):
    ...
    qualidade_sono = Column(
        Integer, CheckConstraint("qualidade_sono BETWEEN 1 AND 10"), nullable=True
    )

    # Energia ao longo do dia
    energia_manha = Column(
        Integer, CheckConstraint("energia_manha BETWEEN 1 AND 10"), nullable=True
    )
    energia_tarde = Column(
        Integer, CheckConstraint("energia_tarde BETWEEN 1 AND 10"), nullable=True
    )
    energia_noite = Column(
        Integer, CheckConstraint("energia_noite BETWEEN 1 AND 10"), nullable=True
    )
